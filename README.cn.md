...
- `GET /fallingts_mdtable/preview?path=` — 按路径或 `@{工作流文件名/ID}` 引用提供本地 image/video/audio 预览(支持 Range); 引用先按 `output|input/<工作流文件名>/<ID>.*` 定位, 未命中再在 output/input 及其「编号-名称」子目录(保存类节点按工作流名建的那层)中递归查找
