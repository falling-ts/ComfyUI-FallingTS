...
5. **🔄 刷新**(紧接在「打开数据」下方)— 按 ID 重新查询 md 文件,用磁盘最新值更新表单(md 文件被外部改动后同步)。
...
- `GET /fallingts_mdtable/preview?path=` — 按路径或 `@{表文件名/ID}` 引用提供本地 image/video/audio 预览(支持 Range); 引用分两趟解析: **严格优先** —— 先在 `output|input/<表文件名>/<ID>.*` 精确找; 全都未命中再**兜底**在 output/input 及其「编号+分隔符+名称」子目录(保存类节点建的那层, 优先 md 表文件名)中按 ID 递归查找, 同族目录(如 `0020_场景首帧` → `00200_场景首帧_QI2.1`)优先于其它目录
