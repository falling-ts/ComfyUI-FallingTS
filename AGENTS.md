...
└── web/
    └── js/                     # 前端扩展脚本 (经 GET /extensions 加载,不参与前端打包)
        ...
        ├── switch.js                   # 分组开关前端联动
        ├── composite.js                # 多图合成: total 动态端口 (image1..8) + 旧版 7 槽 widgets_values 加载时自动迁移
...
| composite | FallingTSImageComposite | 多图合成 (total 驱动: total 1..8 默认 4, 决定左侧 image1..8/标注1..8 端口, **前端按 total 动态增删 image_i 端口 (未启用端口不进提交载荷)**, 与 switch/route/fanout/selector 同套机制; 图 image1..8 (optional, 未连接/None = 该格用底色空白占位, **total 图全空 → 输出本节点最近一次合成结果(sticky), 从未合成则输出 None**) + 标注1..8 (可连线, 默认 前面/右面/后面/左面/上面/下面/近处/远处, 空串 = 不画); 网格列数 = ceil(sqrt(total)) 行优先填充 (total=4 → 2×2, 与旧版布局一致), 统一尺寸 (取最大高宽), 每张子图左上角 CJK 白字黑描边标注, 合成单张图; 字号/间距/底色 None=默认 8/6/#000000, 底色非法值回退黑色) |
...
- **数据类 —— 合成**(composite):total 驱动张数(None → 默认 4, clamp 1..8);image1..8 全部 optional,经 `_first_frame` 统一归一化:None / 空 tuple / list / 零批张量 / 非张量 一律按无值处理 → **该格用底色空白占位**(部分有值时正常合成, 缺格用底色占位);**total 张图全无值 → 输出本节点最近一次合成结果(sticky), 从未合成则输出 None**(绝不崩溃);label1..8 None → 各自默认标注(空串 = 不画);font_size/padding/background_color None → 默认 8.0/6/#000000;
