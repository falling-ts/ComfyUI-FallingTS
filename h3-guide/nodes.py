...
此外补一道**同帧去重**: 4025 的帧索引由「首帧硬钉第 0 帧 + 中间帧按右侧的 `关键帧n所在秒数`
逐列换算 (`max(0, min(round(秒数 x 24), length - 1))`) + 尾帧取 `length - 1`」确定, 两个中间
帧秒数相同或换算后落在同一帧时, 两个互相矛盾的画面会被钉在同一时刻 —— 这正是「条件互相冲突
导致物件漂移」的成因。本节点发现本次新增的图片锚点与上游某一列撞帧时, **撤掉本次的图片锚点并
告警**(音频锚点不占位, 图音同帧仍合法), 让链路跑完而不是产出矛盾条件; 图片本身仍作为参考图
参与条件不受影响。

4025-关键帧视频 用 9 路本节点串联: 9 个槽位 (首帧 / 中间帧 / 尾帧) 填了几个就锚几个点, 空槽
自动少锚点。
"""
...
class FallingTSH3AddGuideNode(io.ComfyNode):
    ...
    def _drop_duplicate_image_anchor(positive):
        ...
        logging.warning(
            "[FallingTSH3AddGuide] 第 %s 帧已被上游某一槽位锚定, 本槽跳过图片锚定以免同一时刻"
            "钉上两个互相矛盾的画面 (本槽图片仍作为参考图参与条件)。通常是两个中间帧列的"
            "「关键帧n所在秒数」相同或过于接近 (24fps 下相差不足半帧即落到同一帧) —— 把本列"
            "秒数改到别的时点, 或清空本槽的图片让该槽不锚定。",
            frame,
        )
