...
**原理(2026-08 重构,缓存式)**:
- 节点后端默认返回 **`ExecutionBlocker` 阻塞下游**;
- **Run**(默认):前端先 `POST /proceed/reset` 把所有继续节点重置为阻塞,再**全量提交**工作流(官方随机化正常推进种子)→ 生成段执行,在第一个继续节点停下;
- 点「**继续**」:前端 `POST /proceed/continue/{id}` 放行该节点,并**冻结所有 `control_after_generate`**(防止官方随机化推进上游种子),再全量重提 → 上游命中缓存跳过,只跑新放行段,到下一个继续节点再停;
- 重复点「继续」逐段跑到底。
