# audio-trim/nodes.py
"""FallingTSAudioTrim 节点: 始终预览(temp), 可在节点上按波形截段(多段), 点「保存」写 output。
...
class FallingTSAudioTrimNode(IO.ComfyNode):
    @classmethod
    ...
    def execute(cls, audio, filename_prefix: str = "audio", filename_suffix: str = "", format: str = "flac", quality: str = "128k") -> IO.NodeOutput:
        ...
        logging.info(
            "[FallingTS][FallingTSAudioTrim] execute 进入: unique_id=%r hidden=%r audio=%s",
...
async def _handle_reset(request: web.Request) -> web.Response:
    """HTTP 路由: 重置所有 FallingTSAudioTrim 为未完成(前端默认 Run / 无段点「完成」时调用)。
...
async def _handle_save(request: web.Request) -> web.Response:
    ...
    参数:
        request (web.Request): POST /audio-trim/save/{node_id}, body 为 JSON
...
PromptServer.instance.routes.post("/audio-trim/save/{node_id}")(_handle_save)
PromptServer.instance.routes.post("/audio-trim/segment/{node_id}")(_handle_segment)
PromptServer.instance.routes.post("/audio-trim/segment-remove/{node_id}")(_handle_segment_remove)
PromptServer.instance.routes.post("/audio-trim/done/{node_id}")(_handle_done)
PromptServer.instance.routes.post("/audio-trim/reset")(_handle_reset)
PromptServer.instance.routes.post("/audio-trim/clear")(_handle_clear)
PromptServer.instance.routes.get("/audio-trim/waveform/{node_id}")(_handle_waveform)
PromptServer.instance.routes.get("/audio-trim/audio-url/{node_id}")(_handle_audio_url)
