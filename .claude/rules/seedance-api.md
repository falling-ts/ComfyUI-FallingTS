...
> 跳转 [响应参数](https://www.volcengine.com/docs/82379/1520757#oCS1tULg)
...
<div data-tips="true" data-tips-type="warning">Seedance 2.0 系列模型不支持直接上传含有真人人脸的参考图/视频。为了便利创作者对肖像的使用，平台推出了以下解决方案，详情参见 <a href="https://www.volcengine.com/docs/82379/2291680#5c67c9a1">教程</a>。</div>
...
* <div data-tips="true" data-tips-type="tip">更多使用技巧：提示词的详细使用技巧，请参见 <a href="https://www.volcengine.com/docs/82379/2222480">Seedance 提示词指南</a>。</div>
...
方舟平台信任 Seedance 2.0 系列模型生成的含人脸视频，您可使用 **本账号下近30天内由上述模型生成的含人脸原始视频** ，作为输入素材进行二次创作，详情参见 [教程](https://www.volcengine.com/docs/82379/2291680#341d7f71)。
...
基于样片任务 ID，生成正式视频。仅 Seedance 1.5 Pro 支持该功能。[阅读](https://www.volcengine.com/docs/82379/1366799#5acd28c8)[文档](https://www.volcengine.com/docs/82379/1366799#5acd28c8) 获取 draft 功能的使用教程和注意事项。
...
使用分为两步：Step1: 调用本接口生成 Draft 视频。Step2: 如果确认 Draft 视频符合预期，可基于 Step1 返回的 Draft 视频任务 ID，调用本接口生成最终视频。[阅读文档](https://www.volcengine.com/docs/82379/1366799#5acd28c8) 获取详细教程。
...
* true：返回生成视频的尾帧图像。设置为 `true` 后，可通过 [查询视频生成任务接口](https://www.volcengine.com/docs/82379/1521309) 获取视频的尾帧图像。尾帧图像的宽高像素值与生成的视频保持一致，无水印。

   使用该参数可实现生成多个连续视频：以上一个生成视频的尾帧作为下一个视频任务的首帧，快速生成多个连续视频，调用示例详见 [教程](https://www.volcengine.com/docs/82379/1366799#141cf7fa)。
...
* default：在线推理模式，RPM 和并发数配额较低（详见 [模型列表](https://www.volcengine.com/docs/82379/1330310#7571da3f)），适合对推理时效性要求较高的场景。

* flex：离线推理模式，TPD 配额更高（详见 [模型列表](https://www.volcengine.com/docs/82379/1330310#7571da3f)），价格为在线推理的 50%， 适合对推理时延要求不高的场景。
...
控制是否开启样片模式。[阅读文档](https://www.volcengine.com/docs/82379/1366799#5acd28c8) 获取使用教程和注意事项。
...
* web_search：联网搜索工具。[阅读教程](https://www.volcengine.com/docs/82379/2291680#c40ed3ef) 获取详细代码示例。
...
* <div data-tips="true" data-tips-type="tip">实际搜索次数可通过 <a href="https://www.volcengine.com/docs/82379/1521309">查询视频生成任务 API</a> 返回的 usage.tool_usage. <strong>web_search</strong> 字段获取，如果为 0 表示未搜索。</div>
...
* <div data-tips="true" data-tips-type="warning">不同模型，可能对应支持不同的参数与取值，详见 <a href="https://www.volcengine.com/docs/82379/1366799#9fe4cce0">输出视频格式</a>。当输入的参数或取值不符合所选的模型时，该参数将被忽略或触发报错：</div>
...
* <div data-tips="true" data-tips-type="tip">4K 视频采用 H.265 编码，少数播放环境可能不兼容，如遇问题，建议升级系统、更换设备或使用 VLC、MPV、QuickTime Player 等<a href="https://www.volcengine.com/docs/82379/2291680#4k_player">播放器</a>查看。</div>
...
<div data-tips="true" data-tips-type="warning">当配置 <strong>ratio</strong> 为 <code>adaptive</code> 时，模型会根据生成场景自动适配宽高比；实际生成的视频宽高比可通过 <a href="https://www.volcengine.com/docs/82379/1521309">查询视频生成任务 API</a> 返回的 <strong>ratio</strong> 字段获取。</div>
...
Note：图生视频，选择的宽高比与您上传的图片宽高比不一致时，方舟会对您的图片进行裁剪，裁剪时会居中裁剪，详细规则见 [图片裁剪规则](https://www.volcengine.com/docs/82379/1366799#f76aafc8)。
...
* <div data-tips="true" data-tips-type="warning">智能指定：设置为 <code>-1</code>，表示由模型在有效范围内自主选择合适的视频长度（整数秒）。实际生成视频的时长可通过 <a href="https://www.volcengine.com/docs/82379/1521309">查询视频生成任务 API</a> 返回的 <strong>duration</strong> 字段获取。注意视频时长与计费相关，请谨慎设置。</div>
...
> 跳转 [请求参数](https://www.volcengine.com/docs/82379/1520757#i6I5fT3J)
...
* <div data-tips="true" data-tips-type="tip">仅支持查询最近 7 天的任务记录，时间区间为 [T\-7天, T)，其中 T 为请求发起时刻的 UTC 时间戳（精确到秒）。</div>


* <div data-tips="true" data-tips-type="tip">视频 URL 有效期为 24 小时，请及时下载或转存。</div>



<span id="PuoLIOIT"></span>
## 鉴权
...
错误提示信息，任务成功返回`null`，任务失败时返回错误数据，错误信息具体参见 [错误处理](https://www.volcengine.com/docs/82379/1299023#common-error-codes-agent)。
...
<div data-tips="true" data-tips-type="tip">seedance 2.0 系列模型存在最低 token 用量限制，如果实际 token 用量 ＜ 最低 token 用量，本字段会返回最低 token 用量，平台按最低 token 用量计费。</div>
...
...
----- task succeeded -----
ContentGenerationTask(id='cgt-20260414114820-*****', model='doubao-seedance-2-0-260128', status='succeeded', error=None, content=Content(video_url='https://ark-acg-cn-beijing.tos-cn-beijing.volces.com/doubao-seedance-2-0/02177613894217600000000000000000000ffffac181cef51b2fe.mp4?X-Tos-Algorithm=TOS4-HMAC-SHA256&X-Tos-Credential=<redacted>-beijing%2Ftos%2Frequest&X-Tos-Date=<redacted>&X-Tos-Expires=86400&X-Tos-Signature=<redacted>&X-Tos-SignedHeaders=host', last_frame_url=None, file_url=None), usage=Usage(completion_tokens=411300, total_tokens=411300), subdivisionlevel=None, fileformat=None, frames=None, framespersecond=24, created_at=1776138520, updated_at=1776139219, seed=33608, revised_prompt=None, service_tier='default', execution_expires_after=172800, generate_audio=True, duration=11, ratio='16:9', resolution='720p', draft=False, draft_task_id=None)
```
...
本项目的节点实现在 `nodes/seedance.py` 和 `api/client.py` 中:

| 文件 | 内容 |
|------|------|
| `api/models.py` | Pydantic 请求/响应模型, 定价数据, 模型配置 |
| `api/client.py` | Volcengine API 客户端: `create_seedance_task()`, `poll_seedance_task()`, `download_video()` |
| `nodes/seedance.py` | 4 个 V3 ComfyNode: TextToVideo / ImageToVideo / FirstLastFrame / Reference |
| `plugin.py` | 入口点: NODE_CLASS_MAPPINGS + inject() + comfy_entrypoint |

调用流程:
```
Node.execute() → create_seedance_task() [POST /api/v3/contents/generations/tasks]
                → poll_seedance_task()  [GET .../tasks/{id}, 轮询 9s]
                → download_video()      [下载结果视频]
                → IO.NodeOutput(video)
```

默认选择模型: doubao-seed-2-0-mini-260428
API Key: 1025ef28-fc09-408b-b348-b010bbc556e9