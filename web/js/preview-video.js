...
app.registerExtension({
  ...
  /**
   * 节点定义注册前钩子: 给 PreviewVideo 追加「保存」按钮。
   *
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      /**
       * 「保存」按钮点击处理: 把文件名前缀 POST 到后端, 后端用缓存的视频直接写 output
       * ({filename_prefix}.mp4, 同名覆盖, 无序号), 【不重跑工作流】。
      ...
      node.addWidget("button", "保存", null, async () => {
        ...
        // 需后端用 execute 时实际接收到的值, 否则保存会落到占位符 "video"
        const prefixLinked =
