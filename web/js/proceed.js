 ...
 *
 * 数据完全来自节点自身的缓存(lazy 门控 + _data_cache), 不依赖 ComfyUI 全局节点缓存判断从哪跑。
...
app.registerExtension({
   ...
   * 默认 Run(未显式指定目标节点)时, 先 POST /proceed/reset 重置所有继续节点为阻塞并清空缓存,
  ...
  async setup() {
    ...
    if (!orig) return;
    /**
     * 包装 queuePrompt: 拦截"默认 Run"(queueNodeIds 为空)分支, 先重置全部继续节点再提交。
     * 分段执行完全靠: lazy 门控(不拉上游) + 节点自身缓存(_data_cache) + partial targets(queue 往下跑),
     * 不依赖 ComfyUI 全局节点缓存 —— 上游是否重跑由 lazy 边决定, 与缓存无关。
     *
